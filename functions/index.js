...
app.post('/api/ai/extract-referral-fields', async (req, res) => {
  try {
    const { referralText } = req.body || {};
    if (!referralText || !String(referralText).trim()) return res.status(400).json({ error: 'Referral text is required', fallback: true });

    const result = await completeJSONPrompt({
      systemPrompt: `You are a structured data extractor for youth residential care Out-of-Home (OOH) referral documents from Nebraska probation offices.
...
- other: final comments, narrative paragraphs, date service needed, preferred community for foster care`,
    ...
    return handleAIError(res, error, 'Failed to extract referral fields');
  }
});
