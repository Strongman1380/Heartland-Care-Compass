...
async function migrateYouthData() {
  try {
    console.log('🚀 Starting youth data migration...');
    ...
    // Clear existing data (remove mock data)
    console.log('🧹 Clearing existing youth data...');
    const deleteResult = await youthCollection.deleteMany({});
    console.log(`✅ Removed ${deleteResult.deletedCount} existing youth records`);
    ...
    console.log(`🎉 Successfully migrated ${convertedYouth.length} youth profiles!`);
...
migrateYouthData().catch(console.error);

export { migrateYouthData, convertYouthToDbFormat };