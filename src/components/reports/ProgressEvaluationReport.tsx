...
export const ProgressEvaluationReport = ({ youth }: ProgressEvaluationReportProps) => {
  ...
  const handleExportPDF = async () => {
    ...
    await exportElementToPDF(printRef.current, `${buildReportFilename(youth, "Progress Evaluation Report")}.pdf`);
  };

  const handleExportDocx = async () => {
    if (!printRef.current) return;
    await exportElementToDocx(printRef.current, `${buildReportFilename(youth, "Progress Evaluation Report")}.docx`);
  };
