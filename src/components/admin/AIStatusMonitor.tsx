...
import { checkAIServiceStatus } from "@/lib/aiClient";
...
export const AIStatusMonitor = () => {
  ...
  const getStatusColor = () => {
    ...
    if (status.available && status.status === 'operational') return 'bg-green-500';
  ...
  const getStatusIcon = () => {
    ...
    if (status.available && status.status === 'operational') return <CheckCircle className="h-4 w-4" />;
