import { initializeApp } from 'firebase/app';
import { getFirestore, collection, getDocs, query, limit, collectionGroup } from 'firebase/firestore';

const defaultFirebaseConfig = {
  apiKey: 'AIzaSyAsLC5kOkGO7YcwxmAFxG91sFGckCYSxaE',
  authDomain: 'heartland-boys-home-data.firebaseapp.com',
  projectId: 'heartland-boys-home-data',
  storageBucket: 'heartland-boys-home-data.firebasestorage.app',
  messagingSenderId: '30882060333',
  appId: '1:30882060333:web:aa84a93fd3257b689d80a4',
  measurementId: 'G-ZMQXFX80T0',
};

const app = initializeApp(defaultFirebaseConfig);
const db = getFirestore(app);

async function check() {
  console.log('Checking youth...');
  const youthSnap = await getDocs(query(collection(db, 'youth'), limit(5)));
  console.log(`Found ${youthSnap.size} youth`);
  
  console.log('Checking behavior_points...');
  const bpSnap = await getDocs(query(collectionGroup(db, 'behavior_points'), limit(5)));
  console.log(`Found ${bpSnap.size} behavior_points`);
  
  console.log('Checking case_notes...');
  const cnSnap = await getDocs(query(collectionGroup(db, 'case_notes'), limit(5)));
  console.log(`Found ${cnSnap.size} case_notes`);
  
  console.log('Checking daily_ratings...');
  const drSnap = await getDocs(query(collectionGroup(db, 'daily_ratings'), limit(5)));
  console.log(`Found ${drSnap.size} daily_ratings`);
  
  process.exit(0);
}

check().catch(err => { console.error(err); process.exit(1); });
